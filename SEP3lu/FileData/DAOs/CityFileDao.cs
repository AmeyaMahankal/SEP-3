...
public class CityFileDao: ICityDao
{
    ...
    public Task<IEnumerable<City>> GetAsync(SearchCityParametersDto dto)
    {
        ...
        {
            result = context.Citys.Where(u => u.Name.Equals(dto.NameContains, StringComparison.OrdinalIgnoreCase));
        }
        if (dto.Name!=null)
        {
    ...
    {
        City? existing = context.Citys.FirstOrDefault(city => city.Id == id);
        if (existing==null)
        {
